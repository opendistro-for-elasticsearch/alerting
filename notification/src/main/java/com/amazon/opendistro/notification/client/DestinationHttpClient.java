...
package com.amazon.opendistro.notification.client;
...
public class DestinationHttpClient {
    ...
    public String execute(BaseMessage message) throws Exception {
        CloseableHttpResponse response = getHttpResponse(message);
    ...
    private CloseableHttpResponse getHttpResponse(BaseMessage message) throws Exception {
        ...
        if (message instanceof CustomWebhookMessage) {
            ...
            if(headerParams == null || headerParams.isEmpty()) {
                ...
                for (Map.Entry<String, String> e : customWebhookMessage.getHeaderParams().entrySet()) httpPostRequest.setHeader(e.getKey(), e.getValue());
            }
        } else {
            uri = buildUri(message.getUrl(), null, null, -1, null, null);
        }
