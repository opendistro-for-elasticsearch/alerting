...
import com.amazon.opendistro.model.CronSchedule
import com.amazon.opendistro.model.ScheduledJob
import com.amazon.opendistro.model.SearchInput
import com.amazon.opendistro.alerting.MonitoringRestTestCase
...
class MonitorRestApiIT : MonitoringRestTestCase() {
    ...
    fun `test query a monitor that doesn't exist`() {
        ...
        val searchResponse = client().makeRequest("GET", "$ALERTING_BASE_URI/_search", emptyMap(), NStringEntity(search, ContentType.APPLICATION_JSON))
        assertEquals("Search monitor failed", RestStatus.OK, searchResponse.restStatus())
        val xcp = createParser(XContentType.JSON.xContent(), searchResponse.entity.content)
        val hits = xcp.map()["hits"]!! as Map<String, Any>
        val numberDocsFound = hits["total"]
    ...
    fun `test query a monitor with UI metadata from Kibana`() {
        ...
        val search = SearchSourceBuilder().query(QueryBuilders.termQuery("_id", monitor.id)).toString()
        val header = BasicHeader(HttpHeaders.USER_AGENT, "Kibana")
        val searchResponse = client().makeRequest("GET", "$ALERTING_BASE_URI/_search", emptyMap(), NStringEntity(search, ContentType.APPLICATION_JSON), header)
        assertEquals("Search monitor failed", RestStatus.OK, searchResponse.restStatus())
    ...
    fun `test disable monitoring`() {
        val settings = client().getClusterSettings(mapOf("include_defaults" to "true")).opendistroSettings()
        assertEquals("Monitoring not enabled", "true", settings?.get("enabled"))

        val updateResponse = client().makeRequest("POST", "$ALERTING_BASE_URI/settings",
                emptyMap(),
        ...
        val responseMap = updateResponse.asMap().stringMap("persistent")?.stringMap("opendistro")?.stringMap("monitoring")
        assertEquals("Setting was not updated in response call.", "false", responseMap?.get("enabled"))

        val newSettings = client().getClusterSettings(mapOf("include_defaults" to "true")).stringMap("persistent")?.stringMap("opendistro")?.stringMap("monitoring")
        assertEquals("Setting was not updated.", "false", newSettings?.get("enabled"))
    }

    @Throws(Exception::class)
    fun `test disable monitoring with non boolean`() {
        val currentSettings = client().getClusterSettings(mapOf("include_defaults" to "true")).opendistroSettings()
        assertEquals("Monitoring not enabled", "true", currentSettings?.get("enabled"))

        try {
            client().makeRequest("POST", "$ALERTING_BASE_URI/settings",
    ...
    fun `test monitor stats no jobs`() {
        val monitorStatsResponse = client().makeRequest("GET", "/_opendistro/_monitors/stats")
