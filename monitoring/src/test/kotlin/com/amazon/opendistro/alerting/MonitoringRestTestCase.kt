...
abstract class MonitoringRestTestCase : ESRestTestCase() {
    ...
    protected fun createAlert(alert: Alert): Alert {
        val response = client().makeRequest("POST", "/.opendistro-alerts/_doc?refresh=true&routing=${alert.monitorId}",
    ...
    protected fun executeMonitor(monitorId: String, params: Map<String, String> = mapOf()) : Response =
            client().makeRequest("POST", "$ALERTING_BASE_URI/$monitorId/_execute", params)
    ...
    fun Map<String, Any>.opendistroSettings(): Map<String, Any>? {
        ...
        return map["defaults"]?.get("opendistro")?.get("monitoring")
    }
