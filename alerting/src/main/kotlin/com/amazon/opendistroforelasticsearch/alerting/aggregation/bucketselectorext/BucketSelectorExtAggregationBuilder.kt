    ...
    companion object {
        ...
        fun parse(reducerName: String, parser: XContentParser): BucketSelectorExtAggregationBuilder {
            ...
            if (bucketsPathsMap == null) {
                throw ParsingException(
                    parser.tokenLocation, "Missing required field [" +
                            PipelineAggregator.Parser.BUCKETS_PATH.preferredName + "] for bucket_selector aggregation [" + reducerName + "]"
                )
            }
            if (script == null) {
                throw ParsingException(
                    parser.tokenLocation, "Missing required field [" +
                            Script.SCRIPT_PARSE_FIELD.preferredName + "] for bucket_selector aggregation [" + reducerName + "]"
                )
            }
            ...
            if (parentBucketPath == null) {
                throw ParsingException(
                    parser.tokenLocation, "Missing required field [" +
                            PARENT_BUCKET_PATH + "] for bucket_selector aggregation [" + reducerName + "]"
                )
            }
