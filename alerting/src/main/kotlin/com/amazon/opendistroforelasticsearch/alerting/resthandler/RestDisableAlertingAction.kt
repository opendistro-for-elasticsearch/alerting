 ...
 */
class RestDisableAlertingAction(settings: Settings, controller: RestController) : BaseRestHandler(settings) {

    init {
        // Acknowledge alerts
        controller.registerHandler(RestRequest.Method.POST, "${AlertingPlugin.MONITOR_BASE_URI}/settings", this)
    }

    override fun getName(): String {
        return "disable_alerting_action"
    }
            ...
            override fun buildResponse(response: ClusterUpdateSettingsResponse): RestResponse {
                if (!response.isAcknowledged) {
                    return BytesRestResponse(
                            ...
                            channel.newBuilder().startObject().field("acknowledged", false).endObject())
                }
    ...
    private fun parseSetting(xcp: XContentParser): Settings {
        val enabledKeySetting = AlertingSettings.ALERTING_ENABLED.key
        val map = xcp.map()
