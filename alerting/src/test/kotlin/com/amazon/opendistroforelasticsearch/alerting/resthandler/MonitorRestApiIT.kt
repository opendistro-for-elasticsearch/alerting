...
class MonitorRestApiIT : AlertingRestTestCase() {
    ...
    fun `test disable alerting`() {
        val settings = client().getClusterSettings(mapOf("include_defaults" to "true")).opendistroSettings()
        assertEquals("Alerting not enabled", "true", settings?.get("enabled"))

        val updateResponse = client().makeRequest("POST", "$ALERTING_BASE_URI/settings",
                emptyMap(),
                StringEntity(
                        jsonBuilder().startObject().field(AlertingSettings.ALERTING_ENABLED.key, false).endObject().string(),
                        ContentType.APPLICATION_JSON))
        val responseMap = updateResponse.asMap().stringMap("persistent")?.stringMap("opendistro")?.stringMap("alerting")
        assertEquals("Setting was not updated in response call.", "false", responseMap?.get("enabled"))

        val newSettings = client()
                .getClusterSettings(mapOf("include_defaults" to "true"))
                .stringMap("persistent")
                ?.stringMap("opendistro")
                ?.stringMap("alerting")
        assertEquals("Setting was not updated.", "false", newSettings?.get("enabled"))
    }

    @Throws(Exception::class)
    fun `test disable alerting with non boolean`() {
        val currentSettings = client().getClusterSettings(mapOf("include_defaults" to "true")).opendistroSettings()
        assertEquals("Alerting not enabled", "true", currentSettings?.get("enabled"))

        try {
            client().makeRequest("POST", "$ALERTING_BASE_URI/settings",
                    StringEntity(
                            jsonBuilder().startObject().field(AlertingSettings.ALERTING_ENABLED.key, "foobarbaz").endObject().string(),
                            ContentType.APPLICATION_JSON))
    ...
    @Throws(Exception::class)
    fun `test disable alerting with wrong key`() {
        val currentSettings = client().getClusterSettings(mapOf("include_defaults" to "true")).opendistroSettings()
        assertEquals("Alerting not enabled", "true", currentSettings?.get("enabled"))
