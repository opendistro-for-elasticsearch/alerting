...
class JobSweeper(private val settings: Settings,
    ...
    @Volatile private var sweeperEnabled: Boolean?  = clusterService.clusterSettings.get("opendistro.monitoring.enabled").get(settings) as Boolean
    ...
    fun getJobSweeperMetrics(): com.amazon.opendistro.JobSweeperMetrics {
        if(!isSweepingEnabled()) {
            return com.amazon.opendistro.JobSweeperMetrics(-1, true)
        }
        val elapsedTime = getFullSweepElapsedTime()
