...
import com.amazon.opendistro.JobSweeperMetrics
import com.amazon.opendistro.model.ScheduledJob
import com.amazon.opendistro.schedule.JobScheduler
import com.amazon.opendistro.schedule.JobSchedulerMetrics
...
class ScheduledJobsStatsTransportAction : TransportNodesAction<ScheduledJobsStatsRequest, ScheduledJobsStatsResponse,
    ...
    @Inject
    constructor(settings: Settings, threadPool: ThreadPool,
                clusterService: ClusterService, transportService: TransportService,
                actionFilters: ActionFilters, indexNameExpressionResolver: IndexNameExpressionResolver,
                jobSweeper: JobSweeper, jobScheduler: JobScheduler) :
            super(settings, ScheduledJobsStatsAction.NAME, threadPool,
                    clusterService, transportService, actionFilters,
                    indexNameExpressionResolver, { ScheduledJobsStatsRequest() },
                    { ScheduledJobStatusRequest() }, ThreadPool.Names.MANAGEMENT, ScheduledJobStats::class.java) {
    ...
    override fun newResponse(request: ScheduledJobsStatsRequest, responses: MutableList<ScheduledJobStats>, failures: MutableList<FailedNodeException>): ScheduledJobsStatsResponse {
        val isExists = clusterService.state().metaData().hasIndex(ScheduledJob.SCHEDULED_JOBS_INDEX)
        ...
        return ScheduledJobsStatsResponse(clusterService.clusterName, responses, failures, isExists, indexHealth)
    }
    ...
    private fun evaluateStatus(jobsInfo: List<JobSchedulerMetrics>, jobSweeperMetrics: JobSweeperMetrics): ScheduledJobStats.ScheduleStatus {
